    ...
    respond_to do |format|
      format.html do
        ...
        if @site.geographic_context_country_id
          ...
          sql="select r.id,r.name,count(distinct pa.project_id) as count,r.center_lon as lon,r.center_lat as lat,
          ...
          sql="select c.id,c.name,count(distinct pa.project_id) as count,c.center_lon as lon,c.center_lat as lat,
               extract(year from p.start_date) as start_year,
               extract(year from p.end_date) as end_year,
               CASE WHEN count(distinct pa.project_id) > 1 THEN
                 '#{@carry_on_url}'||c.id
