class DataSourcesController < ApplicationController
  ...
  def request_export
    ...
    @projects_custom_find_options.merge!({data_source: params[:id],
                                           :time_window   => {
                                             :left  => params[:time_window_left],
                                             :right => params[:time_window_right]
                                           }})

    Resque.enqueue(DataExporter, current_user.id, @site.id, params[:export_format], { data_source: params[:id] })
    render :nothing => true
  end
  ...
  def show
    ...
    @data = DataSource.find params[:id]
    ...
    @data_sources_project_count = @data.total_projects(@site, @filter_by_location)
    ...
    if @filter_by_location.present?
      ...
      @filter_name = "#{@data_source_project_count} projects in #{@location_name}"
    end
        ...
        if @site.geographic_context_country_id
          ...
          sql="select r.id,r.name,count(distinct pa.project_id) as count,r.center_lon as lon,r.center_lat as lat,r.name,
              ...
              from regions as r
                ...
                inner join data_sources_projects as pa on pa.project_id=p.id and pa.data_source_id=#{params[:id].sanitize_sql!.to_i}
                #{location_filter}
