...
class Disease < ActiveRecord::Base
  has_and_belongs_to_many :projects, :join_table => "diseases_projects"
  ...
  def projects_regions(site)
    Region.find_by_sql(<<-SQL
      select r.id,r.name,r.level, r.parent_region_id, r.path, r.country_id,count(ps.*) as count from regions as r
        ...
        inner join diseases_projects as pse on pse.project_id=ps.project_id and pse.disease_id=#{self.id}
  ...
  def total_regions(site)
    ...
    inner join diseases_projects as ps on ps.project_id=psi.project_id
    where ps.disease_id=#{self.id}"
  ...
  def total_projects(site, location_id = nil)
    ...
    sql = "select count(distinct(ps.project_id)) as count from diseases_projects as ps
