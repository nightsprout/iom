class DataSource < ActiveRecord::Base
  has_and_belongs_to_many :projects, :join_table => "data_sources_projects"
  ...
  def projects_regions(site)
    Region.find_by_sql(<<-SQL
      select r.id,r.name,r.level, r.parent_region_id, r.path, r.country_id,count(ps.*) as count from regions as r
        ...
        inner join data_sources_projects as pse on pse.project_id=ps.project_id and pse.data_source_id=#{self.id}
  ...
  def total_regions(site)
    ...
    inner join data_sources_projects as ps on ps.project_id=psi.project_id
  ...
  def total_projects(site, location_id = nil)
    ...
    sql = "select count(distinct(ps.project_id)) as count from data_sources_projects as ps
